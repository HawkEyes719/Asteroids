...
{
	// stop emitting when last particle emits
	...
	const glm::mat4& a_cameraTransform) 
{
	...
	// checks if all particles are dead when we want to stop emitting
	if (m_firstDead == 0 && stopEmitting == true)
		allDead = true;

	// spawn particles
	m_emitTimer += a_deltaTime;
	while (m_emitTimer > m_emitRate && stopEmitting == false)
	{
		emit();
...
void ParticleEmitter::draw(const glm::mat4& a_cameraTransform)
{
	glUseProgram(m_programID);

	int loc = glGetUniformLocation(m_programID, "projectionView");
	glUniformMatrix4fv(loc, 1, GL_FALSE,
		glm::value_ptr(a_cameraTransform));
	...
	// draw particles
	glBindVertexArray(m_vao);
	glDrawElements(GL_POINTS, m_firstDead * 6, GL_UNSIGNED_INT, 0);
}