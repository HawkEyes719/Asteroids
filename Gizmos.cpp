...
void Gizmos::addTransform(const glm::mat4& a_transform, float a_fScale /* = 1.0f */) {
	...
	addLine(glm::vec3(a_transform[3]), glm::vec3(vXAxis), vRed, vRed);
	addLine(glm::vec3(a_transform[3]), glm::vec3(vYAxis), vGreen, vGreen);
	addLine(glm::vec3(a_transform[3]), glm::vec3(vZAxis), vBlue, vBlue);
}
...
void Gizmos::addAABB(const glm::vec3& a_center, 
	...
	{
		vX = glm::vec3((*a_transform * glm::vec4(vX, 0)));
		vY = glm::vec3((*a_transform * glm::vec4(vY, 0)));
		vZ = glm::vec3((*a_transform * glm::vec4(vZ,0)));
	}
...
void Gizmos::addCylinderFilled(const glm::vec3& a_center, float a_radius, float a_fHalfLength,
	...
	for ( unsigned int i = 0 ; i < a_segments ; ++i )
	{
		...
		if (a_transform != nullptr)
		{
			v0top = glm::vec3((*a_transform * glm::vec4(v0top, 0)));
			v1top = glm::vec3((*a_transform * glm::vec4(v1top, 0)));
			v2top = glm::vec3((*a_transform * glm::vec4(v2top, 0)));
			v0bottom = glm::vec3((*a_transform * glm::vec4(v0bottom, 0)));
			v1bottom = glm::vec3((*a_transform * glm::vec4(v1bottom, 0)));
			v2bottom = glm::vec3((*a_transform * glm::vec4(v2bottom, 0)));
		}
...
void Gizmos::addRing(const glm::vec3& a_center, float a_innerRadius, float a_outerRadius,
	...
	for ( unsigned int i = 0 ; i < a_segments ; ++i )
	{
		...
		if (a_transform != nullptr)
		{
			v1outer = glm::vec3((*a_transform * glm::vec4(v1outer, 0)));
			v2outer = glm::vec3((*a_transform * glm::vec4(v2outer, 0)));
			v1inner = glm::vec3((*a_transform * glm::vec4(v1inner, 0)));
			v2inner = glm::vec3((*a_transform * glm::vec4(v2inner, 0)));
		}
...
void Gizmos::addSphere(const glm::vec3& a_center, float a_radius, int a_rows, int a_columns, const glm::vec4& a_fillColour, 
	...
	for (int row = 0; row <= a_rows; ++row)
	{
		...
		for ( int col = 0; col <= a_columns; ++col )
		{
			...
			if (a_transform != nullptr)
			{
				v4Point = glm::vec3((*a_transform * glm::vec4(v4Point, 0)));
				v4Normal = glm::vec3((*a_transform * glm::vec4(v4Normal, 0)));
			}
...
void Gizmos::addCapsule(const glm::vec3& a_center, float a_height, float a_radius,
	...
	glm::vec3 topCenter = a_center + glm::vec3(top);
	glm::vec3 bottomCenter = a_center + glm::vec3(bottom);
	...
	for (int row = 0; row <= (a_rows); ++row) {
		...
		for (int col = 0; col <= a_cols; ++col) {
			...
			if (a_rotation != nullptr) {
				v4Point = glm::vec3(*a_rotation * glm::vec4(v4Point, 0));
				v4Normal = glm::vec3(*a_rotation * glm::vec4(v4Normal, 0));
			}
	...
	for (int i = 0; i < a_cols; ++i) {
		...
		addTri(topCenter + glm::vec3(pos1), bottomCenter + glm::vec3(pos1), bottomCenter + glm::vec3(pos), a_fillColour);
		addTri(topCenter + glm::vec3(pos1), bottomCenter + glm::vec3(pos), topCenter + glm::vec3(pos), a_fillColour);

		addLine(topCenter + glm::vec3(pos), topCenter + glm::vec3(pos1), white, white);
		addLine(bottomCenter + glm::vec3(pos), bottomCenter + glm::vec3(pos1), white, white);
		addLine(topCenter + glm::vec3(pos), bottomCenter + glm::vec3(pos1), white, white);
	}
}
...
void Gizmos::add2DTriangle(const glm::vec2& a_center, const glm::vec2& a_extents, const glm::vec4& a_colour, const glm::mat4* a_transform /*= nullptr*/) {
	glm::vec2 verts[3];
	...
	if (a_transform != nullptr)
	{
		vX = glm::vec2(*a_transform * glm::vec4(vX, 0, 0));
		vY = glm::vec2(*a_transform * glm::vec4(vY, 0, 0));
	}
...
void Gizmos::add2DCircle(const glm::vec2& a_center, float a_radius, unsigned int a_segments, const glm::vec4& a_colour, const glm::mat4* a_transform /*= nullptr*/) {
	...
	for ( unsigned int i = 0 ; i < a_segments ; ++i )
	{
		...
		if (a_transform != nullptr)
		{
			v1outer = glm::vec2(*a_transform * glm::vec4(v1outer,0,0));
			v2outer = glm::vec2(*a_transform * glm::vec4(v2outer,0,0));
		}
